...
void blf_encipher(blowfish_context *ctx, u_int32_t *xl, u_int32_t *xr) {
    u_int32_t *s_box = ctx->s_box[0];
    u_int32_t *p_box = ctx->p_box;
...
void blf_expand_0_state(blowfish_context *ctx, const u_int8_t *key, u_int16_t k_bytes) {
    ...
    for (i = 0; i < 4; ++i) {
        for (u_int16_t k = 0; k < 256; k += 2) {
            ...
            ctx->s_box[i][k] = data_l;
            ctx->s_box[i][k + 1] = data_r;
        }
    }
}

void blf_expand_state(blowfish_context *ctx, const u_int8_t *data, u_int16_t d_bytes, const u_int8_t *key,
                      u_int16_t k_bytes) {
    u_int16_t i, j = 0;
...
void blf_init_state(blowfish_context *c) {
    static constexpr u_int32_t s_init[4][256] = {
        {
    ...
    memcpy(&c->s_box, s_init, sizeof(s_init));

    static constexpr u_int32_t p_init[18] = {
    ...
    memcpy(&c->p_box, p_init, sizeof(p_init));
}
